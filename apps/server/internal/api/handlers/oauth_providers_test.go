...
func TestListProvidersReportsConfiguredProviders(t *testing.T) {
	...
	require.Equal(t, "pixelfed", out[6].Platform)
	require.Equal(t, "Pixelfed", out[6].DisplayName)
	require.Equal(t, "peertube", out[7].Platform)
	require.Equal(t, "lemmy", out[8].Platform)
	require.Equal(t, "piefed", out[9].Platform)
	require.Equal(t, "linkedin", out[10].Platform)
	require.Equal(t, "threads", out[11].Platform)
